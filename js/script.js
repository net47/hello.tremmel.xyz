...
$(document).ready(function() {
	...
	for(i in linkArray) {
		...
		if(line.substr(0,4) != 'http') {
			if(count > 1)
				html = html + '</div>';
				html = html + '<div class="block"><h1>' + line + '</h1><ul class="fa-ul">';
				count++;
				continue;
		}
		...
		var icon = lineArray[2];

		/*  Add HTML code  *\
		\*=================*/
		if(newwindow)
			html = html + '<li><i class="fa-li fa ' + icon + '"></i><a href="' + url + '" target="_blank">' + title + '</a></li>'
		else
			html = html + '<li><i class="fa-li fa ' + icon + '"></i><a href="' + url + '">' + title + '</a></li>'
	}
